...
## Decision
`Cascadia` hosts V2 at the root (ICM-structured) and the V1 prototype verbatim under `v1/` (excluding its `.git` and Emergent `.gitconfig`), marked read-only by `v1/CONTEXT.md`. No code is ported by copy; ideas are ported via `docs/V1_AUDIT.md`. The private `cascadia-papsukkal` repository remains the immutable original.
