...
| `corpus/` | The flood-science corpus: twelve domain files (runoff generation, snow/ROS, atmospheric rivers, orographic precipitation, antecedent conditions, routing/hydraulics, compound-coastal, flood statistics, regulation/operations, measured non-stationarity, forecasting/verification, cascading hazards) plus `CROSS-DOMAIN-FINDINGS.md` — convergences, contradictions, the forcing-to-stage causal chain, and the factor ranking. Start at `corpus/README.md` | mixed, per file — 8 of 12 carry an adversarial re-check; 2 headline computations did **not** reproduce and say so in place; `atmospheric-rivers.md` was never re-checked. Per-file status in the corpus index |
...
| `cesium-cinematic-repos-and-skills-2026-09-01.md` | Inventory of GitHub repos, libraries and Claude Code skills for the cinematic renderer (camera tours, post-processing, water, wind/precip, imagery-fade status, resium, tile-pyramid tooling, public mosaics incl. NASA GIBS probes, R2/Workers serving) with quoted licenses, stars/last-push, and adopt/adapt/skip verdicts; flags Cesium 1.144's new `ScreenSpaceMapCameraController` | yes — every Cesium symbol grepped in the installed 1.144 d.ts/source; licenses quoted from LICENSE files via GitHub API; GIBS tiles probed |
